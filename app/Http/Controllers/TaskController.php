...
class TaskController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            // Send email to the assigned user
                ...
                if ($user && $project) {
                    Mail::to($user->email)->queue(new ProjectAssignedMail($user, $project));
                }
                dispatch(function() use ($validated, $task) {
    ...
    public function update(Request $request, $id)
    {
        try {
            ...
            // If only updating the description
            if ($request->has('task_description') && $request->keys() === ['task_description']) {
                $validated = $request->validate([
                    'task_description' => 'nullable|string',
                ]);
        ...
        try {
            ...
            if ($isSubtask) {
                ...
                activity('task')
                    ...
                    ->log("Deleted subtask: {$task->task_name}");
                } else {
                    activity('task')
                        ->causedBy(auth()->user())
                        ->performedOn($task)
            ...
            $response = [
                'message' => $isSubtask ? 'Subtask deleted successfully' : 'Task deleted successfully',
    ...
    public function getSortingView(Request $request)
    {
        ...
        return response()->json($sortingView);
    }
    ...
    public function batchSave(Request $request, $projectId)
    {
        ...
        foreach ($tasks as $taskData) {
            ...
            // Set project_id and assigned_to if not present
            $taskData['project_id'] = $projectId;
            if (!isset($taskData['assigned_to']) && isset($taskData['assignedTo'])) {
            ...
            // Send notification/email for new tasks or if assigned_to changed
            if ($isNewTask || $assignedToChanged) {
                $user = $task->assignedUser;
                $project = $task->project;
                if ($user && $project) {
                    // Send email
                ...
                $this->notificationService->createForUser(
                    ...
                    'task_assigned',
                    'New Task Assigned',
                    "You have been assigned a task: {$task->task_name} in project: {$project->proj_name}",
                    route('projects.show', $task->project_id)
                );
            }
            ...
            $toDelete = array_diff($existingSubtaskIds, $updatedSubtaskIds);
            foreach ($toDelete as $delId) {
                Task::find($delId)?->delete();
            }

            $results[] = $task->fresh(['subtasks']);
        }

        return response()->json(['success' => true, 'tasks' => $results]);
    }
}
