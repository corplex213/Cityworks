function saveTask(tableWrapper) {
    ...
    rows.forEach(row => {
        ...
        if (!startDate || !dueDate) {
            ...
            validationMessages.push(`Task "${taskNameInput.value}" must have both a start date and a due date.`);
            return;
        }
        ...
        while (nextRow && nextRow.classList.contains('subtask-row')) {
            ...
            if (!subtaskStartDate || !subtaskDueDate) {
                hasValidationError = true;
                validationMessages.push(`Subtask "${subtaskNameInput.value}" must have both a start date and a due date.`);
                nextRow = nextRow.nextElementSibling;
                continue;
            }
        ...
        if (hasValidationError) {
            alert("Some tasks have invalid dates:\n\n" + validationMessages.join('\n') + "\n\nPlease ensure that the due date is not earlier than the start date for any task or subtask.");
            return;
        }
       ...
       // --- Batch Save: Send all tasks in one request ---
        if (window.showGlobalLoading) window.showGlobalLoading();

        fetch(`/projects/${projectId}/tasks/batch-save`, {
            method: 'POST',
            headers: {
        ...
        .catch(error => {
            if (window.hideGlobalLoading) window.hideGlobalLoading();
            alert(error.message || 'An error occurred while saving the tasks.');
        });
}
    ...
    function deleteComment(commentId) {
        ...
        .then(data => {
            ...
            if (commentsContainer) {
                newCount = Array.from(commentsContainer.querySelectorAll('[data-comment-id]'))
                    .filter(el => !el.closest('[id^="replies-"]')).length;
            }
            if (typeof updateCommentCount === 'function') updateCommentCount(newCount);
        })
    ...
    function deleteFile(fileId) {
        ...
        .then(data => {
            ...
            const newCount = fileList ? fileList.querySelectorAll('[data-file-id]').length : 0;
            if (typeof updateFileCount === 'function') updateFileCount(newCount);
        })
