...
function initializeKanban() {
    ...
    document.querySelectorAll('.task-list').forEach(list => {
        list.addEventListener('dragover', e => {
            ...
            // Get the dragged card and check if it belongs to this kanban
            const taskId = e.dataTransfer.getData('text/plain');
            if (!taskId) return; // During dragover, we can't access the data yet
            
            // Instead, get card ownership from the card element's dataset
            const draggedCard = document.querySelector('.dragging');
            if (!draggedCard) return;
            
            const cardUserId = draggedCard.getAttribute('data-user-id');
            const targetKanbanId = list.closest('.kanban-container').id;
            const targetUserId = targetKanbanId.replace('kanban-', '');
            
            // Check if card can be dropped in this kanban
            if (cardUserId && targetUserId && cardUserId !== targetUserId) {
                // Different user's kanban - show not-allowed cursor
                e.dataTransfer.dropEffect = 'none';
                list.classList.add('invalid-drop-target');
                list.classList.remove('drag-over');
            } else {
                // Same user's kanban - show move cursor
                e.dataTransfer.dropEffect = 'move';
                list.classList.add('drag-over');
                list.classList.remove('invalid-drop-target');
            }
        });
        ...
        list.addEventListener('drop', e => {
            ...
            // Check if this drop is allowed (same user's kanban)
            const cardUserId = originalCard.getAttribute('data-user-id');
            const targetKanbanId = list.closest('.kanban-container').id;
            const targetUserId = targetKanbanId.replace('kanban-', '');

            if (cardUserId && targetUserId && cardUserId !== targetUserId) {
                console.log('Cannot move tasks between different users');
                return; // Don't allow the drop
            }

            // Remove ALL instances of this card from ALL columns
            document.querySelectorAll(`[data-task-id="${taskId}"]`).forEach(card => {
                card.remove();
            });

            // Remove placeholder if present in the target column
            const placeholder = list.querySelector('.kanban-placeholder');
            if (placeholder) placeholder.remove();

            const newStatus = list.getAttribute('data-status');

            // Update the server and create a new card in the target column
            updateTaskStatus(taskId, newStatus)
                .then(updatedTask => {
                    ...
                    // Refresh main table (if you have such a function)
...
function updateTaskStatus(taskId, newStatus) {
    return new Promise((resolve, reject) => {
        // First fetch the complete task to get all required fields
        fetch(`/tasks/${taskId}`)
            .then(response => {
            ...
            .then(task => {
                ...
                // Update only the status but keep all other fields
                task.status = backendStatus;
                
                // Send the complete task object back to the server
                ...
                .then(data => {
                    ...
                    resolve(task); // Return the task data
                });
            })
            .catch(error => {
...
window.loadTasksIntoKanban = function(tasks, userId) {
    ...
    document.querySelectorAll(`#kanban-${userId} .task-list`).forEach(list => {
        if (list.children.length === 0) {
            let placeholderText = '';
            switch (list.getAttribute('data-status')) {
                case 'To Do':
                    placeholderText = 'No tasks for Checking.';
                    break;
                case 'In Progress':
                    placeholderText = 'No tasks for Revision.';
                    break;
                case 'Review':
                    placeholderText = 'No tasks Deferred.';
                    break;
                case 'Done':
                    placeholderText = 'No tasks Completed.';
                    break;
                default:
...
function createTaskCard(task) {
    ...
    taskTitle.style.wordBreak = 'break-word';
    taskTitle.style.overflowWrap = 'break-word'; 
    taskTitle.style.whiteSpace = 'normal';
    taskTitle.style.overflow = 'visible';
    taskTitle.style.marginBottom = '0.75rem';
    taskTitle.style.lineHeight = '1.3';

    if (task.assigned_user && task.assigned_user.id) {
        taskCard.setAttribute('data-user-id', task.assigned_user.id);
    }

    if (task.subtasks) {
        taskCard._subtasksData = task.subtasks;
    }
