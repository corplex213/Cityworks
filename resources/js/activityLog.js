function getActionType(description) {
    ...
    // Task actions
    if (description.startsWith('created task')) return 'created';
    if (description.startsWith('updated task')) return 'updated';
    if (description.startsWith('deleted task')) return 'deleted';

    // Subtask actions
    if (description.startsWith('added subtask')) return 'created';
    if (description.startsWith('updated subtask')) return 'updated';
    if (description.startsWith('deleted subtask')) return 'deleted';

    // Engineer/user actions
    if (description.startsWith('added engineer')) return 'created';
    if (description.startsWith('removed engineer')) return 'deleted';
    if (description.startsWith('registered user')) return 'created';
    if (description.startsWith('deleted user')) return 'deleted';
    if (description.startsWith('approved user')) return 'updated';

    // File actions
    if (description.startsWith('uploaded file')) return 'created';
    if (description.startsWith('deleted file')) return 'deleted';

    // Comment actions
    if (description.startsWith('added comment')) return 'created';
    if (description.startsWith('deleted comment')) return 'deleted';
    if (description.startsWith('replied to comment')) return 'created';
    if (description.startsWith('mentioned user')) return 'created';
    if (description.startsWith('edited comment')) return 'updated';
...
function loadActivityLog() {
    ...
    function createActivityElement(activity) {
        ...
        const actionType = getActionType(activity.description);
        ...
        const icon = getActivityTypeIcon(actionType);
        let changesHtml = '';

        // Fields to exclude from log display
        const excludedFields = ['assigned_to', 'project_id', 'parent_task_id'];
        ...
        if (activity.properties) {
            ...
            // For deleted actions, show only old values
            if (Object.keys(attrs).length === 0 && Object.keys(old).length > 0 && actionType === 'deleted') {
                Object.keys(old).forEach(key => {
                    if (excludedFields.includes(key)) return;
                    if (
                        old[key] === null ||
                        old[key] === undefined ||
                        (typeof old[key] === 'string' && old[key].trim() === '')
                    ) {
                        return;
                    }
                    let oldValue = old[key];
                    if (['start_date', 'due_date'].includes(key) && old[key]) {
                ...
                // For created/updated actions, show new (and possibly old) values
                Object.keys(attrs).forEach(key => {
                    ...
                    if (old[key] !== undefined && old[key] !== attrs[key]) {
                        ...
                        `;
                    } else if (Object.keys(old).length === 0 && actionType === 'created') {
                        // Created value
                        changesHtml += `
                            <div class="flex items-center text-sm text-gray-600 dark:text-gray-300">
                                <span class="font-medium capitalize mr-2">${key.replace(/_/g, ' ')}:</span>
        ...
        return `
            <div class="relative pl-8 mb-8 last:mb-0" data-type="${actionType}">
