let TEAM_MEMBERS_CACHE = null;
let commentAttachments = [];

document.addEventListener('DOMContentLoaded', () => {

    // Attach input event to all comment/reply textareas
    function attachMentionInputHandler(textarea) {
        textarea.addEventListener('input', function() {
            const cursorPos = textarea.selectionStart;
            const textBeforeCursor = textarea.value.substring(0, cursorPos);
            const lastAtIndex = textBeforeCursor.lastIndexOf('@');
            const suggestionsId = textarea.id === 'newComment'
                ? 'mentionSuggestions'
                : `mentionSuggestions-reply-${textarea.id.replace('replyText-', '')}`;
            const suggestionsDiv = document.getElementById(suggestionsId);
            if (!suggestionsDiv) return;
            if (
                lastAtIndex === -1 ||
                textBeforeCursor.substring(lastAtIndex, cursorPos).indexOf(' ') !== -1
            ) {
    ...
    const mainTextarea = document.getElementById('newComment');
    if (mainTextarea) attachMentionInputHandler(mainTextarea);

    // Attach to all reply textareas
    document.querySelectorAll('textarea[id^="replyText-"]').forEach(attachMentionInputHandler);
});
...
const closeTaskDetailsDrawer = () => taskDrawerHandler.close();
                        ...
                        <div class="flex items-start space-x-3">
                            ...
                            <div class="flex-grow">
                                <textarea 
                                    ...
                                    onkeydown="handleCommentKeydown(event, ${comment.id})"></textarea>
    ...
    function handleCommentKeydown(event, commentId = null) {
        const isReply = commentId !== null;
        const textareaId = isReply ? `replyText-${commentId}` : 'newComment';
        const suggestionsId = isReply ? `mentionSuggestions-reply-${commentId}` : 'mentionSuggestions';

        const textarea = document.getElementById(textareaId);
        const suggestionsDiv = document.getElementById(suggestionsId);

        if (event.key === '@') {
            showMentionSuggestions(commentId);
            ...
            if (!suggestionsDiv.classList.contains('hidden')) {
                ...
                !suggestionsDiv.classList.contains('hidden')) {
            event.preventDefault();
            ...
            setTimeout(() => {
                ...
                const textBeforeCursor = textarea.value.substring(0, cursorPos);
                const lastAtIndex = textBeforeCursor.lastIndexOf('@');
                // Hide suggestions if "@" is not present before the cursor
                if (lastAtIndex === -1) {
                    suggestionsDiv.classList.add('hidden');
                } else if (textBeforeCursor.substring(lastAtIndex, cursorPos).indexOf(' ') === -1) {
                    showMentionSuggestions(commentId);
                } else {
                    suggestionsDiv.classList.add('hidden');
                }
    ...
    function showMentionSuggestions(commentId = null) {
        const isReply = commentId !== null;
        const textareaId = isReply ? `replyText-${commentId}` : 'newComment';
        const suggestionsId = isReply ? `mentionSuggestions-reply-${commentId}` : 'mentionSuggestions';
        const textarea = document.getElementById(textareaId);
        ...
        let query = '';
        if (lastAtIndex !== -1) {
            query = textBeforeCursor.substring(lastAtIndex + 1).trim();
        }
        ...
        function renderSuggestions(members) {
            ...
            suggestionsDiv.innerHTML = filtered.length
                ...
                `).join('')
                : `<div class="p-3 text-gray-400">No users found</div>`;
        }

        if (TEAM_MEMBERS_CACHE) {
            renderSuggestions(TEAM_MEMBERS_CACHE);
        } else {
            fetch('/api/team-members')
                .then(response => response.json())
                .then(members => {
                    TEAM_MEMBERS_CACHE = members;
                    renderSuggestions(members);
                })
                .catch(() => {
                    suggestionsDiv.innerHTML = '<div class="p-3 text-red-500">Error loading users</div>';
                });
        }
    }

    function insertMention(name, textareaId = 'newComment') {
        // Handle both main comments and replies
        const isReply = textareaId !== 'newComment';
        const textarea = document.getElementById(isReply ? `replyText-${textareaId}` : textareaId);
        const suggestionsId = isReply ? `mentionSuggestions-reply-${textareaId}` : 'mentionSuggestions';
    ...
    function addComment() {
        ...
        .then(data => {
            ...
            document.getElementById('selectedCommentFiles').textContent = '';
...
function loadFiles() {
    ...
    function handleCommentAttachment(event) {
        commentAttachments = Array.from(event.target.files);
        // Show selected file names (optional)
        const names = commentAttachments.map(f => f.name).join(', ');
        document.getElementById('selectedCommentFiles').textContent = names;
    }
    window.handleCommentAttachment = handleCommentAttachment;
