...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function initializeCalendar() {
        ...
        calendar = new Calendar(calendarEl, {
            ...
            if (isEvent) {
                // Show delete button for events
                if (deleteBtn) deleteBtn.style.display = '';

                // Only show assigned user and the event's actual date (not today's date)
                if (modalProject) modalProject.parentElement.style.display = 'none';
                if (modalPriority) modalPriority.parentElement.style.display = 'none';
                if (modalStatusDot) modalStatusDot.style.display = 'none';

                // Show assigned user
                if (modalAssigned) {
                    modalAssigned.textContent = info.event.extendedProps.assigned_to || 'Unassigned';
                    modalAssigned.parentElement.style.display = '';
                }

                // Show event's start and end date
                if (modalDate) {
                ...
                // Show all details for tasks
                if (modalProject) {
                    modalProject.textContent = info.event.extendedProps.project || 'No project';
                    modalProject.parentElement.style.display = '';
                }
                if (modalPriority) {
                    modalPriority.textContent = info.event.extendedProps.priority || 'No priority';
                    modalPriority.parentElement.style.display = '';
                }
                if (modalAssigned) {
                    modalAssigned.textContent = info.event.extendedProps.assigned_to || 'Unassigned';
                    modalAssigned.parentElement.style.display = '';
                ...
                if (modalStatusDot) {
                    modalStatusDot.style.display = '';
                    // Set status dot color as before
                    const status = info.event.extendedProps.status || '';
                    modalStatusDot.className = 'w-3 h-3 rounded-full mr-2';
                    if (status === 'Completed') modalStatusDot.classList.add('bg-green-500');
                    else if (status === 'For Checking') modalStatusDot.classList.add('bg-blue-500');
                    else if (status === 'For Revision') modalStatusDot.classList.add('bg-yellow-400');
                    else if (status === 'Deferred') modalStatusDot.classList.add('bg-red-500');
                    else modalStatusDot.classList.add('bg-gray-400');
                }
                // Show date as usual
                const start = info.event.start;
    ...
    function updateSidebarForCurrentUser() {
        // Always fetch events for the current user only
        fetch('/calendar/events?userFilter=mine&all=1')
            .then(res => res.json())
            .then(allEvents => {
