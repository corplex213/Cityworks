...
window.Echo.channel('projects')
    .listen('ProjectCreated', (e) => {
        console.log('ProjectCreated event received:', e);
        location.reload();
    })
    .listen('ProjectUpdated', (e) => {
        console.log('ProjectUpdated event received:', e);
        location.reload();
    })
    .listen('ProjectDeleted', (e) => {
        alert('ProjectDeleted received!');
        console.log('ProjectDeleted event received:', e);
        location.reload();
    });
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // JavaScript for Modal and Dropdown
        ...
        function filterProjects() {
            ...
            // Show or hide the "No activities found" row
            if (noResultsRow) {
                noResultsRow.style.display = (!found && searchValue) ? '' : 'none';
            }
        }
    ...
    function closeArchiveModal() {
        ...
        function toggleStatusDropdown(projectId, event) {
            ...
            if (dropdown.classList.contains('hidden')) {
                ...
                // Use a handler that ignores the button and dropdown itself
                ...
                // Attach the event after a tick so the opening click doesn't close it
                setTimeout(() => {
                    document.addEventListener('click', closeDropdownOnClickOutside);
                }, 20);
            } else {
