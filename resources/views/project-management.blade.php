<x-app-layout>
    ...
    <x-slot name="header">
        ...
        <div class="max-w-8xl mx-[-35px] sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 shadow-lg rounded-lg">
                <div class="p-7 text-gray-900 dark:text-gray-100">
                    ...
                    <p class="mb-4"><strong>Type of Activity:</strong> {{ $project->proj_type}}</p>
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                window.quill = new Quill('#taskDescriptionEditor', {
                    theme: 'snow',
                    modules: {
                        toolbar: [
                            [{ header: [1, 2, false] }],
    ...
    <div id="projectOverviewSection" class="bg-gray-50 dark:bg-gray-900 p-6 rounded-lg shadow-lg mx-6 lg:mx-12" style="margin-left: 0px;margin-right: 0px;">
        ...
        <section id="mainTableSection" class="group-section mt-8">
            ...
            <div class="relative bg-white dark:bg-gray-800 p-4 shadow">
                <div id="mainTableLoading" class="flex flex-col items-center justify-center h-64 bg-gray-100 dark:bg-gray-800 rounded-lg shadow-lg" style="display: none;">
                    ...
                    <h3 class="text-xl font-semibold text-gray-500 dark:text-gray-400">Loading tasks...</h3>
                </div>
                ...
                <div class="p-4 h-full flex flex-col">
                    ...
                    <div class="flex justify-between items-center mb-4 pb-2 border-b border-gray-700">
                        ...
                        <div class="flex items-center space-x-2">
                            ...
                            <button
                                id="deleteTaskButton"
                                onclick="deleteTaskFromDrawer()"
                                class="text-red-400 hover:text-red-500 {{ auth()->user()->can('delete tasks') ? '' : 'cursor-not-allowed opacity-50' }}"
                                @if(!auth()->user()->can('delete tasks')) style="pointer-events: none;" @endif
                            >
                    ...
                    <div id="taskDetailsContent" class="flex-1 overflow-y-auto space-y-4">
                        ...
                        <div class="mb-8">
                            <div class="flex items-center mb-3 gap-2">
                                ...
                                <label class="text-gray-100 text-lg font-bold tracking-wide">Task Description</label>
                            </div>
                                <div class="relative group">
                                    @can('edit tasks')
                                        <!-- Read-only view -->
                                        <div
                                            class="transition-all duration-300 ease-in-out p-5 rounded-xl bg-gray-800/80 border border-gray-700 text-gray-100 shadow-sm min-h-[56px] hover:shadow-lg group group-hover:ring-2 group-hover:ring-blue-400 flex items-center justify-between cursor-text"
                                            id="taskDescriptionReadonly"
                                            style="word-break:break-word;"
                                            ondblclick="enableTaskDescriptionEdit()"
                                            title="Double-click to edit description"
                                        >
                                            <div id="taskDescriptionContent" class="flex-1 pr-4">
                                                {!! $task->task_description ?? '<span class="text-gray-400">No description provided.</span>' !!}
                                            </div>
                                        </div>
                                        <!-- Edit view (hidden by default) -->
                                        <div id="taskDescriptionEditorWrapper" class="transition-all duration-300 ease-in-out mt-2 hidden">
                                            <div id="taskDescriptionEditor" class="rounded-xl overflow-hidden border border-blue-400 bg-white dark:bg-gray-900"></div>
                                            <div class="flex justify-end items-center mt-3 gap-2">
                                                <button
                                                    class="px-5 py-2 bg-gray-600 hover:bg-gray-700 text-white font-semibold rounded-lg shadow transition"
                                                    onclick="showTaskDescriptionReadonly(window.quill.root.innerHTML)">
                                                    Cancel
                                                </button>
                                                <button
                                                    id="saveTaskDescBtn"
                                                    class="px-6 py-2 bg-blue-600 hover:bg-blue-700 text-white font-semibold rounded-lg shadow transition flex items-center justify-center"
                                                    onclick="saveTaskDescription(window.quill.root.innerHTML)">
                                                    <span id="saveBtnText">Save</span>
                                                    <svg id="saveSpinner" class="hidden animate-spin ml-2 h-5 w-5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                                                        <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                                                        <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8z"></path>
                                                    </svg>
                                                </button>
                                    ...
                                    @else
                                        <div class="p-5 rounded-xl bg-gray-800/80 border border-gray-700 text-gray-100 shadow-sm min-h-[56px]">
                                            {!! $task->task_description ?? '<span class=\'text-gray-400\'>No description provided.</span>' !!}
                                        </div>
                                    @endcan
                                </div>
                                            ...
                                            <div class="flex items-start space-x-3">
                                                ...
                                                <div class="flex-grow">
                                                    <div class="relative">
                                                        ...
                                                        <!-- Add this div for mention highlighting preview -->
                                                        <div id="mentionPreview" class="absolute inset-0 w-full p-3 text-gray-200 pointer-events-none hidden whitespace-pre-wrap"></div>
                                                    ...
                                                    <div class="mt-3 flex justify-end items-center space-x-2">
                                                        ...
                                                        <label for="commentAttachment" class="flex items-center cursor-pointer bg-gray-700 hover:bg-gray-600 text-white px-3 py-2 rounded-lg transition-colors duration-200 shadow-sm mr-2">
                                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.172 7l-6.586 6.586a2 2 0 102.828 2.828l7.07-7.07a4 4 0 00-5.656-5.657l-7.07 7.07a6 6 0 108.485 8.485l6.586-6.586" />
                                                            </svg>
                                                            Add Attachment
                                                            <input id="commentAttachment" type="file" class="hidden" multiple onchange="handleCommentAttachment(event)">
                                                        </label>
                                                        <!-- Show selected file names (optional) -->
                                                        <span id="selectedCommentFiles" class="text-xs text-gray-400"></span>
                                                        <!-- Post Comment Button -->
                                                        <button 
                                                            onclick="addComment()" 
                            ...
                            <div class="space-y-5">
                                ...
                                <div class="bg-gray-800 rounded-lg p-4">
                                    <div class="flex items-center justify-center w-full">
                                        <label
                                            for="fileUpload"
                                            class="flex flex-col items-center justify-center w-full h-32 border-2 border-gray-600 border-dashed rounded-lg
                                                bg-gray-700 hover:bg-gray-600
                                                {{ auth()->user()->can('upload files to tasks') ? 'cursor-pointer' : 'cursor-not-allowed opacity-50' }}"
                                            @if(!auth()->user()->can('upload files to tasks')) style="pointer-events: none;" @endif
                                        >
                                ...
                                <!-- File List Header -->
                                <div class="flex items-center space-x-2 px-2">
                                    <div class="flex-shrink-0 w-6 h-6 bg-green-500/20 rounded-md flex items-center justify-center">
                                ...
                                <div id="noFilesMessage" class="hidden text-center py-8">
                                    <div class="w-16 h-16 bg-gray-700/50 rounded-full mx-auto flex items-center justify-center mb-4">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-8 w-8 text-gray-500" viewBox="0 0 20 20" fill="currentColor">
                                            <path fill-rule="evenodd" d="M4 4a2 2 0 012-2h4.586A2 2 0 0112 2.586L15.414 6A2 2 0 0116 7.414V16a2 2 0 01-2 2H6a2 2 0 01-2-2V4z" clip-rule="evenodd" />
                                        </svg>
                                    </div>
                                    <h4 class="text-gray-400 font-medium">No files attached</h4>
                                    <p class="text-gray-500 text-sm mt-1">Upload files to share with the team</p>
                                </div>
                            </div>
                        </div>
                        </div>
            ...
            <div id="activityLogDrawer" class="fixed inset-y-0 right-0 w-full sm:w-1/2 bg-white dark:bg-gray-800 shadow-lg transform translate-x-full transition-transform duration-200 ease-out z-50">
                <div class="flex flex-col h-full">
                    ...
                    <!-- Activity Content -->
                    <div id="activityLogContent" class="flex-1 overflow-y-auto p-6 relative border-l-2 border-gray-200 dark:border-gray-700">
        ...
        <div id="kanbanSection" class="hidden">
            @if($users->isEmpty())
                <div class="kanban-global-placeholder flex flex-col items-center justify-center h-64 bg-gray-100 dark:bg-gray-800 rounded-lg shadow-lg my-8">
                    <h3 class="text-2xl font-semibold text-gray-500 dark:text-gray-400">Start to see your Kanban by assigning a task to engineer!</h3>
                    <p class="text-base text-gray-400 dark:text-gray-500">Assign an engineer in the Main Table to create a Kanban board.</p>
                </div>
            @endif
            @foreach($users as $user)
            ...
            <div class="px-8 pt-8 pb-2">
                ...
                <div class="divide-y divide-gray-200 dark:divide-gray-700">
                    @php
                        ...
                        $grouped = [
                            'Administrative Engineer' => $adminPositions,
                            'Managerial Engineer' => $managerPositions,
                            'Staff Engineer' => $staffPositions,
                        ];
                    @endphp

                    @foreach($grouped as $groupLabel => $positions)
                        @php
                            $usersInGroup = $allUsers->whereIn('position', $positions);
                            $groupKey = \Illuminate\Support\Str::slug($groupLabel, '_');
                        @endphp
                        @if($usersInGroup->count())
                            <div class="py-4">
                                <div class="flex items-center justify-between mb-2">
                                    <span class="font-semibold text-gray-700 dark:text-gray-200 text-base">{{ $groupLabel }}</span>
                                    <button type="button"
                                        class="toggle-group-btn text-gray-400 hover:text-blue-500 focus:outline-none transition"
                                        data-group="{{ $groupKey }}">
                                        <span id="icon-{{ $groupKey }}" class="text-2xl leading-none">-</span>
                                    </button>
                                </div>
                                <ul class="space-y-2 group-user-list" id="group-{{ $groupKey }}">
                                    @foreach($positions as $pos)
                                        @foreach($allUsers->where('position', $pos) as $user)
                                            <li>
                                                <button 
                                                    data-user-id="{{ $user->id }}" 
                                                    class="w-full flex items-center gap-2 text-left bg-gray-100 dark:bg-gray-700 hover:bg-blue-100 dark:hover:bg-blue-900 text-gray-800 dark:text-gray-200 px-4 py-2 rounded-lg transition"
                                                    onclick="closeUserModal(); addUserTable({{ \Illuminate\Support\Js::from($user->name) }}, {{ $user->id }})"
                                                >
                                                    <span class="font-medium">{{ $user->name }}</span>
                                                    <span class="text-xs text-gray-400">({{ $user->position }})</span>
                                                </button>
                                            </li>
                                        @endforeach
                                    @endforeach
                ...
                <div class="mt-6 flex justify-end">
                    <button onclick="closeUserModal()" class="bg-gray-500 hover:bg-gray-600 text-white px-5 py-2 rounded-lg font-semibold shadow">Close</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <div id="globalLoadingOverlay" style="display:none; position:fixed; inset:0; background:rgba(0,0,0,0.3); z-index:9999;" class="flex items-center justify-center">
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-lg px-8 py-6 flex flex-col items-center">
            <svg class="animate-spin h-10 w-10 text-blue-500 mb-2" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8z"></path>
            </svg>
            <span class="text-gray-700 dark:text-gray-200 font-semibold">Loading...</span>
        </div>
    </div>
</x-app-layout>
<script>
...
document.addEventListener('DOMContentLoaded', function() {
    document.querySelectorAll('.toggle-group-btn').forEach(function(btn) {
        btn.addEventListener('click', function(event) {
            ...
            var list = document.getElementById('group-' + groupKey);
