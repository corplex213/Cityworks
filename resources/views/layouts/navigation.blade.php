            ...
            <div class="hidden sm:flex sm:items-center sm:space-x-4 sm:ms-6">
                ...
                <div class="relative" x-data="notificationSystem">
                    ...
                    <div x-show="showToast"
                        x-transition
                        class="fixed bottom-6 right-6 w-80 flex items-center gap-3 bg-blue-600 text-white px-6 py-5 rounded-xl shadow-2xl z-[9999]"
                        style="min-height:56px;">
                        <svg class="h-7 w-7 flex-shrink-0 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6 6 0 10-12 0v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9" />
                        </svg>
                        <div class="flex-1">
                            <span class="font-semibold text-lg" x-text="toastMessage"></span>
                            <div class="text-xs mt-1 opacity-80">Click the bell to view details.</div>
                        </div>
                        <button @click="showToast = false" class="ml-2 text-white hover:text-gray-200 focus:outline-none">
                ...
                <div class="flex items-center">
                    <x-dropdown align="right" width="48">
                        ...
                        <x-slot name="content">
                            ...
                            <x-dropdown-link :href="route('personnel')">
                                {{ __('Personnel') }}
                            </x-dropdown-link>
                            @role('administrative')
                            <x-dropdown-link :href="route('userAccessControl')">
        ...
        Alpine.data('notificationSystem', () => ({
            ...
            init() {
                ...
                setTimeout(() => {
                    ...
                    if (window.currentUserId && window.Echo && typeof window.Echo.channel === 'function') {
                        window.Echo.channel('user.' + window.currentUserId)
                            .listen('NotificationCreated', (e) => {
                            console.log('Notification received:', e);
                            this.fetchUnreadCount();
                            this.fetchAllNotifications();
                            this.toastMessage = 'You have received a notification';
                            this.showToast = true;
                            setTimeout(() => this.showToast = false, 3000);
                        });
                    } else {
                        console.error('Echo not initialized or channel not a function');
                    }
