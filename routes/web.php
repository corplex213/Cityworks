...
// Route for Spatie Activity Log
Route::get('/projects/{project}/activities', function (\App\Models\Project $project) {
    $activities = Activity::where('subject_type', \App\Models\Task::class)
        ->where(function ($query) use ($project) {
            // For existing tasks (created/updated): subject_id is in tasks table with this project_id
            $query->whereHasMorph('subject', [\App\Models\Task::class], function ($q) use ($project) {
                $q->where('project_id', $project->id);
            })
            // For deleted tasks: project_id is in properties
            ->orWhere('properties->project_id', $project->id)
            ->orWhereJsonContains('properties->project_id', $project->id);
        })
        ->whereIn('log_name', ['task', 'default'])
        ->with('causer', 'subject')
...
Route::get('/dashboard', [DashboardController::class, 'viewDashboard'])
    ->middleware(['auth', 'verified', EnsureUserIsApproved::class])
    ->name('dashboard');
...
Route::post('/projects/{project}/tasks/batch-save', [TaskController::class, 'batchSave']);
...
// Profile after updating email
Route::get('/profile/verify-email', function () {
...
Route::post('/user-access/register', [UserAccessController::class, 'register'])->name('user-access.register');
Route::post('/user-access-control/{user}/approve', [UserAccessController::class, 'approve'])
    ->middleware(\App\Http\Middleware\CheckUserAccess::class . ':manage user access')
    ->name('user-access.approve');
Route::delete('/user-access/reject/{user}', [UserAccessController::class, 'reject'])->name('user-access.reject');
