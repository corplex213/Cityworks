...
function addKanbanTask(userId, statusKey) {
    ...
    // Build the card HTML to match your main table fields
    let cardHtml = `
        <div class="kanban-add-card bg-white dark:bg-gray-700 rounded-lg shadow p-4 mb-2 flex flex-col space-y-2">
            ...
            <div class="flex space-x-2">
                ...
                <div class="flex-1">
                    ...
                    <select class="status-input w-full p-2 rounded border">
                        <option value="For Checking" ${statusKey === 'To Do' ? 'selected' : ''}>For Checking</option>
                        <option value="For Revision" ${statusKey === 'In Progress' ? 'selected' : ''}>For Revision</option>
                        <option value="Deferred" ${statusKey === 'Review' ? 'selected' : ''}>Deferred</option>
                        <option value="Completed" ${statusKey === 'Done' ? 'selected' : ''}>Completed</option>
                    </select>
                </div>
            </div>
    ...
    cardEl.querySelector('.save-btn').onclick = function() {
        const taskData = {
            ...
            status: cardEl.querySelector('.status-input').value,
