...
<template id="taskCardTemplate">
    <div class="task-card bg-white dark:bg-gray-700 rounded-lg shadow p-4 cursor-move" draggable="true">
        ...
        <button class="toggle-subtasks-btn mt-2 text-xs text-blue-500 hover:underline flex items-center" type="button">
            <span>Show Subtasks</span><span class="subtask-count ml-2 text-xs text-blue-500"></span>
            <svg class="ml-1 w-4 h-4 transition-transform duration-200 arrow-icon" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" d="M19 9l-7 7-7-7"></path>
            </svg>
        </button>
        <div class="subtasks-list mt-2 hidden">
...
.subtask-count {
    margin-left: 2px; /* or 0 if you want no gap */
}
</style>
...
function createTaskCard(task) {
    ...
    const fieldsDiv = taskCard.querySelector('.task-fields');
        ...
        <div class="flex justify-between items-center">
            ...
            <span class="text-sm text-gray-200">${task.budget ? '₱' + parseFloat(task.budget).toLocaleString('en-US', {minimumFractionDigits: 2}) : ''}</span>
        </div>
    ...
    function renderSubtasks(subtasks) {
        ...
        taskCard.querySelector('.subtask-count').textContent = `(${count})`;
        ...
        // Add the "Add Subtask" button at the end
        const addBtn = document.createElement('button');
        addBtn.className = 'add-kanban-subtask-btn mt-2 text-xs text-green-600 hover:underline flex items-center';
        addBtn.type = 'button';
        addBtn.innerHTML = '<span>+ Add Subtask</span>';
    ...
    subtasksVisible = !subtasksVisible;
        if (subtasksVisible) {
            subtasksList.classList.remove('hidden');
            renderSubtasks(task.subtasks);
            toggleBtn.querySelector('span').textContent = 'Hide Subtasks';
            arrowIcon.classList.add('open');
        } else {
            subtasksList.classList.add('hidden');
            toggleBtn.querySelector('span').textContent = 'Show Subtasks';
            arrowIcon.classList.remove('open');
        }
    });

    const subtaskCountSpan = taskCard.querySelector('.subtask-count');
    const initialCount = Array.isArray(task.subtasks) ? task.subtasks.length : 0;
    subtaskCountSpan.textContent = initialCount > 0 ? `(${initialCount})` : '';
