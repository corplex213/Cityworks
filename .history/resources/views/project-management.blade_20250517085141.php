<x-app-layout>
    ...
    <div id="projectOverviewSection" class="bg-gray-50 dark:bg-gray-900 p-6 rounded-lg shadow-lg mx-6 lg:mx-12" style="margin-left: 0px;margin-right: 0px;">
        ...
        <div id="kanbanSection" class="hidden">
            <div id="dynamicKanbanContainer"></div>
        </div>
    </div>
...
<script>
    ...
    function filterTasks() {
        ...
        function addUserTable(userName, userId) {
            ...
            createKanbanForUser(userName, userId);
                ...
                .then(tasks => {
                   ...
                   @foreach($users as $user)
                        if (window.loadTasksIntoKanban) {
                            window.loadTasksIntoKanban(kanbanTasks, {{ $user->id }});
                        }
                    @endforeach
    ...
    function createKanbanForUser(userName, userId) {
        const kanbanContainer = document.getElementById('dynamicKanbanContainer');
        if (document.getElementById(`kanban-wrapper-${userId}`)) return; // Prevent duplicates

        const wrapper = document.createElement('div');
        wrapper.id = `kanban-wrapper-${userId}`;
        wrapper.className = 'user-kanban-wrapper mb-12';
        wrapper.setAttribute('data-user-id', userId);

        const header = document.createElement('h2');
        header.className = 'text-xl font-bold mb-4 text-gray-800 dark:text-gray-200';
        header.textContent = `${userName}'s Kanban`;

        // Render the kanban-board partial via Blade include
        wrapper.innerHTML = header.outerHTML + `@include('kanban-board', ['userId' => '__USER_ID__'])`.replace('__USER_ID__', userId);

        kanbanContainer.appendChild(wrapper);

        // Optionally, load tasks into this Kanban board if needed
        if (window.loadTasksIntoKanban && window.kanbanTasks) {
            window.loadTasksIntoKanban(window.kanbanTasks, userId);
        }
    }
