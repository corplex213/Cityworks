...
function addKanbanTask(userId, statusKey) {
    ...
    card.className = 'kanban-add-card bg-white dark:bg-gray-700 rounded-lg shadow p-4 mb-2 flex flex-col space-y-2';

    card.innerHTML = `
        <input type="text" class="task-title-input w-full p-2 rounded border" placeholder="Task name">
        <input type="date" class="start-date-input w-full p-2 rounded border" placeholder="Start Date">
        <input type="date" class="due-date-input w-full p-2 rounded border" placeholder="Due Date">
        <select class="priority-input w-full p-2 rounded border">
            <option value="Normal">Normal</option>
            <option value="High">High</option>
            <option value="Low">Low</option>
        </select>
        <input type="number" class="budget-input w-full p-2 rounded border" placeholder="Budget">
    ...
    card.querySelector('.save-btn').onclick = function() {
        const taskData = {
            project_id: PROJECT_ID, // Set this variable in your Blade with the current project ID
            assigned_to: userId,
            task_name: card.querySelector('.task-title-input').value,
            start_date: card.querySelector('.start-date-input').value,
            due_date: card.querySelector('.due-date-input').value,
            priority: card.querySelector('.priority-input').value,
            status: statusKey === 'To Do' ? 'For Checking' :
                    statusKey === 'In Progress' ? 'For Revision' :
                    statusKey === 'Review' ? 'Deferred' : 'Completed',
            budget: card.querySelector('.budget-input').value
        };
        ...
        .then(res => res.json())
        .then(data => {
            // Reload Kanban tasks for this user (you may already have this function)
