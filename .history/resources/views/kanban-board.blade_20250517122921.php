...
function addKanbanTask(userId, statusKey) {
    // Find the correct Kanban column's task-list
    const taskList = document.querySelector(`#kanban-${userId} .task-list[data-status="${statusKey}"]`);
    if (!taskList) return;

    // Create a table if not present (Kanban columns use divs, so we need a table for row logic)
    ...
    // Use the main table's addNewRow logic to add a new editable row
    window.addNewRow(tbody);

    // Set default status for the new row based on the Kanban column
    const lastRow = tbody.querySelector('tr:last-child');
    if (lastRow) {
        const statusCell = lastRow.querySelector('.status-cell .status-value');
        if (statusCell) {
            switch (statusKey) {
                case 'To Do':
                ...
                case 'In Progress':
                    ...
                    break;
                case 'Review':
                    statusCell.textContent = 'Deferred';
                    break;
                case 'Done':
