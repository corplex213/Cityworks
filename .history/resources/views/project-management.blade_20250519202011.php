...
<script>
    ...
    function filterTasks() {
        ...
        function deleteTaskFromDrawer() {
            ...
            if (!taskId) {
                if (row) row.remove();
                const kanbanCard = document.querySelector(`.task-card[data-task-id="${taskId}"], .subtask-card[data-task-id="${taskId}"]`);
            ...
            .then(data => {
                if (row) row.remove();
                // Remove from Kanban if present (main task or subtask)
