...
<div id="kanbanSection" class="hidden">
    <div class="kanban-container flex space-x-4 p-4 overflow-x-auto">
        <!-- To Do Column -->
        ...
        <!-- Review Column -->
        <div class="kanban-column bg-gray-100 dark:bg-gray-800 rounded-lg shadow-lg w-80 flex-shrink-0">
            <div class="column-header bg-purple-500 text-white p-4 rounded-t-lg">
                <h3 class="text-lg font-semibold">Review</h3>
                <span class="task-count text-sm bg-purple-600 px-2 py-1 rounded-full">0</span>
            </div>
...
window.loadTasksIntoKanban = function(tasks) {
    // Clear existing tasks
    document.querySelectorAll('.task-list').forEach(list => {
        list.innerHTML = '';
    });

    // Status mapping based on your statuses
    ...
    tasks.forEach(task => {
        ...
        const targetList = document.querySelector(`.task-list[data-status="${mappedStatus}"]`);
        if (targetList) {
...
function createTaskCard(task) {
    ...
    priorityBadge.classList.add(getPriorityColor(task.priority));
...
function openTaskDetails(task) {
    // Find the row in the main table that corresponds to this task
    const row = document.querySelector(`tr[data-task-id="${task.id}"]`);
    if (row) {
        // Trigger the existing task details drawer
        window.openTaskDetails(row);
    }
}
</script> 