    ...
    <div id="projectOverviewSection" class="bg-gray-50 dark:bg-gray-900 p-6 rounded-lg shadow-lg mx-6 lg:mx-12" style="margin-left: 0px;margin-right: 0px;">
        ...
        <div id="kanbanSection" class="hidden">
            @foreach($users as $user)
                <div class="user-kanban-wrapper mb-12" data-user-id="{{ auth()->id() }}">
                    <h2 class="text-xl font-bold mb-4 text-gray-800 dark:text-gray-200">{{ auth()->user()->name }}'s Kanban</h2>
                    @include('kanban-board', ['userId' => auth()->id()])
                </div>
            @endforeach
        </div>
    </div>
