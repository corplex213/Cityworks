...
<template id="taskCardTemplate">
    <div class="task-card bg-white dark:bg-gray-700 rounded-lg shadow p-4 cursor-move" draggable="true">
        ...
        <button class="toggle-subtasks-btn mt-2 text-blue-500 hover:bg-blue-100 rounded-full p-1 transition" type="button" title="Show/Hide Subtasks">
...
function createTaskCard(task) {
    ...
    const updateToggleBtn = () => {
        ...
        // Optionally, update tooltip with subtask count
        const count = task.subtasks && Array.isArray(task.subtasks) ? task.subtasks.length : 0;
        toggleBtn.title = subtasksVisible ? `Hide Subtasks (${count})` : `Show Subtasks (${count})`;
    };
