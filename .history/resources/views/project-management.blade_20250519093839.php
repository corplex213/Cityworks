                ...
                .then(tasks => {
                   ...
                   @foreach($users as $user)
                        if (window.loadTasksIntoKanban) {
                            // Flatten main tasks and subtasks into one array for Kanban
                            const flatTasks = [];
                            userTasks.forEach(task => {
                                flatTasks.push(task);
                                if (Array.isArray(task.subtasks)) {
                                    task.subtasks.forEach(subtask => {
                                        flatTasks.push({
                                            ...subtask,
                                            parent_task_id: task.id, // Ensure parent_task_id is set
                                            assigned_user: subtask.assigned_user // Ensure assigned_user is present
                                        });
                                    });
                                }
                            });
                            window.loadTasksIntoKanban(flatTasks, userId);
                        }
                    @endforeach
