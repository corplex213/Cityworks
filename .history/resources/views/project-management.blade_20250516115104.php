        ...
        <div id="kanbanSection" class="hidden">
            @foreach($users as $user)
                <div class="user-kanban-wrapper mb-12" data-user-id="{{ $user->id }}">
                    <h2 class="text-xl font-bold mb-4 text-gray-800 dark:text-gray-200">{{ $user->name }}'s Kanban</h2>
        ...
        const projectId = {{ $project->id }};
            return fetch(`/projects/${projectId}/tasks`)
                ...
                .then(tasks => {
                    ...
                    // Dynamically render Kanban boards only for users with tables
                    const kanbanSection = document.getElementById('kanbanSection');
                    ...
                    selectedUsers.forEach(userId => {
                        ...
                        let userName = '';
                        Object.values(tasks).forEach(userTasks => {
                            if (String(userTasks[0].assigned_user.id) === String(userId)) {
                                userName = userTasks[0].assigned_user.name;
                            }
                        });
                        // Create wrapper
                        ...
                        wrapper.innerHTML = `
                            <h2 class="text-xl font-bold mb-4 text-gray-800 dark:text-gray-200">${userName}'s Kanban</h2>
                            @include('kanban-board', ['userId' => '__USER_ID__'])
                        `.replace(/__USER_ID__/g, userId);
                        kanbanSection.appendChild(wrapper);

                        // Load tasks into this user's Kanban
                        if (window.loadTasksIntoKanban) {
                            window.loadTasksIntoKanban(kanbanTasks, userId);
                        }
                    });
                   @foreach($users as $user)
                        if (window.loadTasksIntoKanban) {
