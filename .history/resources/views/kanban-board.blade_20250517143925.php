...
// Render subtasks if any
    ...
    addSubtaskBtn.addEventListener('click', function(e) {
        ...
        form.onsubmit = function(ev) {
            ...
            console.log('Saving subtask:', subtaskData); // Debug
    ...
    taskCard.addEventListener('click', () => {
        openTaskDetails(task); // Pass the task object, not the DOM element
    });
...
// Patch openTaskDetails to handle both DOM row and Kanban task object
window.openTaskDetails = function(rowOrTask) {
    // If it's a DOM element (main table)
    if (rowOrTask instanceof HTMLElement) {
        // ...existing code for extracting from DOM row...
        // (No change needed here)
    } else if (typeof rowOrTask === 'object' && rowOrTask !== null) {
        // Kanban: rowOrTask is a task object
        const task = rowOrTask;
        ...
        if (completionTimestampElement) {
            if ((task.status === 'Completed' || task.status === 'completed') && task.id) {
                fetch(`/tasks/${task.id}/completion-time`)
                    .then(response => response.json())
                    .then(data => {
                    ...
                    .catch(() => completionTimestampElement.classList.add('hidden'));
            } else {
                completionTimestampElement.classList.add('hidden');
            }
        }
