...
// Render subtasks if any
    ...
    if (task.subtasks && Array.isArray(task.subtasks) && task.subtasks.length > 0) {
        task.subtasks.forEach(subtask => {
            ...
            subtaskDiv.className = 'subtask-card bg-gray-100 dark:bg-gray-800 p-2 rounded flex items-center justify-between mb-1 ml-4';
        ...
        taskCard.querySelector('.toggle-subtasks-btn').style.display = 'none';
    }
    ...
    toggleBtn?.addEventListener('click', function(e) {
        ...
        this.querySelector('span').textContent = subtasksList.classList.contains('hidden') ? 'Show Subtasks' : 'Hide Subtasks';
    });
    ...
    addSubtaskBtn.addEventListener('click', function(e) {
        ...
        form.className = 'kanban-subtask-form bg-gray-50 dark:bg-gray-900 p-3 rounded mt-2 flex flex-col space-y-2 border border-gray-300 dark:border-gray-700';
        // Prefill values from parent task
        const parentStartDate = task.start_date || '';
        const parentDueDate = task.due_date || '';
        const parentPriority = task.priority || 'Normal';
        // Source of Funding for POW
        let sourceOfFundingHtml = '';
        if (window.projectType === 'POW') {
            sourceOfFundingHtml = `
            <div>
                <label class="block text-xs mb-1">Source of Funding</label>
                <select class="subtask-source-of-funding-input w-full p-2 rounded border mb-2">
        ...
        form.innerHTML = `
            ...
            <input type="date" class="subtask-start-date-input w-full p-2 rounded border" placeholder="Start Date" value="${parentStartDate}">
            <input type="date" class="subtask-due-date-input w-full p-2 rounded border" placeholder="Due Date" value="${parentDueDate}">
            <select class="subtask-priority-input w-full p-2 rounded border">
                <option value="High" ${parentPriority==='High'?'selected':''}>High</option>
                <option value="Normal" ${parentPriority==='Normal'?'selected':''}>Normal</option>
                <option value="Low" ${parentPriority==='Low'?'selected':''}>Low</option>
            </select>
        ...
        // Cancel handler
        form.querySelector('.cancel-subtask-btn').onclick = function() {
            form.remove();
        };
        ...
        if (window.projectType === 'POW') {
            const sourceSelect = form.querySelector('.subtask-source-of-funding-input');
        ...
        form.onsubmit = function(ev) {
            ...
            const taskName = form.querySelector('.subtask-title-input').value.trim();
            if (!taskName) {
            ...
            const subtaskData = {
                ...
                status: task.status, // inherit parent status
                budget: form.querySelector('.subtask-budget-input').value
            };
            if (window.projectType === 'POW') {
                subtaskData.source_of_funding = form.querySelector('.subtask-source-of-funding-input').value;
                subtaskData.other_funding_source = form.querySelector('.other-funding-source input')?.value || null;
            }
            fetch('/tasks', {
                method: 'POST',
                headers: {
...
// Fix openTaskDetails to accept both DOM row and task object
function openTaskDetails(taskOrRow) {
    // If it's a DOM element (row), use as is
    if (taskOrRow instanceof HTMLElement && typeof taskOrRow.getAttribute === 'function') {
        if (window.taskDrawerHandler && typeof window.taskDrawerHandler.open === 'function') {
            window.taskDrawerHandler.open(taskOrRow);
        }
        return;
    }
    // If it's a task object, try to find the row in the main table
    if (taskOrRow && taskOrRow.id) {
        const row = document.querySelector(`tr[data-task-id="${taskOrRow.id}"]`);
        if (row && window.taskDrawerHandler && typeof window.taskDrawerHandler.open === 'function') {
            window.taskDrawerHandler.open(row);
        } else {
            // If not found, fallback: open a drawer or modal for Kanban
            // You can implement a Kanban-specific drawer/modal here if needed
            // For now, just alert
            alert('Task details drawer is only available in the main table view.');
        }
    }
}

// Make sure loadExistingTasks is defined globally for Kanban and table
if (typeof window.loadExistingTasks !== 'function') {
    window.loadExistingTasks = function() {
        // This should trigger a reload of all tasks for both Kanban and table views
        // You may need to implement this to fetch tasks from the server and call loadTasksIntoKanban for each user
        // Example placeholder (replace with your actual AJAX logic):
        fetch(`/projects/${PROJECT_ID}/tasks`)
            .then(res => res.json())
            .then(data => {
                // If you have multiple users, loop through users and call loadTasksIntoKanban(tasks, userId)
                // For now, assume a single userId is available in the template
                if (typeof window.loadTasksIntoKanban === 'function') {
                    window.loadTasksIntoKanban(data.tasks, '{{ $userId }}');
                }
            });
    };
}
