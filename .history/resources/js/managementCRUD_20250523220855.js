function saveTask(tableWrapper) {
        ...
        while (nextRow && nextRow.classList.contains('subtask-row')) {
            ...
            if (isSubtaskChanged && subtaskId) {
                ...
                if (subtaskBudgetValue !== Number(subtaskBudgetInput.getAttribute('data-old-value') || 0) && 
                    ...
                    const oldBudgetValue = parseFloat((subtaskBudgetInput.getAttribute('data-old-value') || 0));
                    const newBudgetValue = subtaskBudgetValue;
                    const epsilon = 0.001;
                    if (Math.abs(oldBudgetValue - newBudgetValue) > epsilon) {
                        changes.budget = {
            ...
            subtaskBudgetInput.setAttribute('data-old-value', subtaskBudgetValue.toString());
            if (projectType === 'POW') {
    ...
    Promise.all(savePromises)
        .then(() => {
            ...
            // Refresh activity log if drawer is open
            const activityDrawer = document.getElementById('activityLogDrawer');
            if (activityDrawer && !activityDrawer.classList.contains('translate-x-full')) {
                loadActivityLog(); // Reload the activity logs
            }
        })
        .catch(() => {
            alert('An error occurred while saving the tasks. Please try again.');
        });
}
