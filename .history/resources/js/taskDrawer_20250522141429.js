...
const closeTaskDetailsDrawer = () => taskDrawerHandler.close();
        ...
        commentDiv.innerHTML = `
            <div class="flex items-start space-x-3">
                ...
                <div class="flex-grow">
                    ...
                    <div class="mt-2 text-gray-700 dark:text-gray-300 whitespace-pre-wrap">${formatMentions(comment.content)}</div>
    ...
    function handleCommentKeydown(event) {
        ...
        const previewContainer = document.getElementById('mentionPreview');
        
        if (previewContainer) {
            // Update the preview with highlighted mentions
            setTimeout(() => {
                const text = textarea.value;
                if (text.includes('@')) {
                    previewContainer.innerHTML = formatMentions(text);
    ...
    function insertMention(name) {
        ...
        if (lastAtIndex !== -1) {
            // Replace the @mention partial text with the full name
    ...
    .then(response => response.json())
    .then(data => {
        // Create and add the new comment element immediately
        if (data.comment) {
            const commentsContainer = document.getElementById('commentsContainer');
            const commentElement = createCommentElement(data.comment);
            
            // Add new comment at the top for better visibility
            ...
            if (document.getElementById('mentionPreview')) {
                document.getElementById('mentionPreview').classList.add('hidden');
            }
            
            // Update comment count
            updateCommentCount(parseInt(document.getElementById('commentCount')?.textContent || '0') + 1);
            
            // Make sure the comments container is visible (hiding any "no comments" message)
    ...
    .finally(() => {
        ...
        if (commentBtn) {
            ...
            commentBtn.innerHTML = `<svg class="h-5 w-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 19l9 2-9-18-9 18 9-2zm0 0v-8"></path></svg> Post Comment`;
        }
    });
}
...
function formatMentions(text) {
    ...
    // Regex to match @username pattern - stops at space or punctuation
    // Removed background styling, kept just the bold and text color
    return safeText.replace(/@([a-zA-Z0-9_]+)/g, 
        '<span class="font-bold text-blue-500 dark:text-blue-300">@$1</span>');
}
