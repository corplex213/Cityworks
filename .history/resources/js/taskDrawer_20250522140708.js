...
function formatMentions(text) {
    ...
    // Get list of valid usernames to check against
    const validUsers = window.VALID_USERS || [];
    
    // Split text by spaces to process each word
    let parts = safeText.split(/(@[a-zA-Z0-9 ]+)/g);
    
    return parts.map(part => {
        // Check if this part starts with @ and might be a mention
        if (part.startsWith('@')) {
            // Extract just the name part (without @)
            const name = part.substring(1).trim();
            
            // Find where the actual username ends (at first space after a valid username)
            const userPart = validUsers.find(user => 
                name.startsWith(user.name) || 
                user.name.startsWith(name)
            );
            
            if (userPart) {
                // Only highlight the actual username
                const username = userPart.name;
                const restOfText = name.substring(username.length);
                
                return `<span class="font-bold text-blue-500 bg-blue-500/10 dark:bg-blue-900/30 dark:text-blue-300 px-1 rounded">@${username}</span>${restOfText}`;
            }
        }
        return part;
    }).join('');
