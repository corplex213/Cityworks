function openTaskDetailsFromKanban(task) {
    ...
    taskTitle.style.maxWidth = 'calc(100% - 60px)';
    taskTitle.style.overflowWrap = 'break-word';
    
    // Format dates in "Month Day" format
    ...
    // Set formatted dates
    document.getElementById('startDate').textContent = formatShortDate(task.start_date);
    document.getElementById('dueDate').textContent = formatShortDate(task.due_date);
    ...
    document.getElementById('assignedBy').textContent = task.assigned_by_name || 'System';
    document.getElementById('assignedTo').textContent = task.assigned_user ? task.assigned_user.name : 'Unassigned';
    
    // Set assignment timestamp if available
    const assignmentTimestampElement = document.getElementById('assignmentTimestamp');
    if (assignmentTimestampElement && task.assigned_at) {
        assignmentTimestampElement.querySelector('span').textContent = 
            new Date(task.assigned_at).toLocaleString('en-US', {
                year: 'numeric',
                month: 'long',
                day: 'numeric',
                hour: '2-digit',
...
window.openTaskDetailsFromKanban = openTaskDetailsFromKanban;
