...
function initializeKanban() {
    ...
    document.querySelectorAll('.task-list').forEach(list => {
        ...
        list.addEventListener('drop', e => {
            ...
            if (originalCard) {
                // Get the task data from the original card
                const taskData = {
                    id: taskId,
                    task_name: originalCard.querySelector('.task-title').textContent,
                    start_date: originalCard.querySelector('.task-fields div:nth-child(1) .text-gray-200').textContent,
                    due_date: originalCard.querySelector('.task-fields div:nth-child(2) .text-gray-200').textContent,
                    budget: originalCard.querySelector('.budget-value').textContent.replace('₱', '').replace(/,/g, ''),
                    priority: originalCard.querySelector('.priority-badge').textContent,
                    status: newStatus,
                    subtasks: []
                };
                
                // If the original has subtasks, keep track of them
                if (originalCard._subtasksData) {
                    taskData.subtasks = originalCard._subtasksData;
                }
                ...
                // Create a new card with the task data
