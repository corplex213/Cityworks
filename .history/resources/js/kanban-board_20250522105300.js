...
function initializeKanban() {
    ...
    document.querySelectorAll('.task-list').forEach(list => {
        ...
        list.addEventListener('drop', e => {
            ...
            if (originalCard) {
                try {
                    // IMPORTANT: Remove the card from the DOM immediately to prevent duplication
                    originalCard.remove();
                    
                    // Then fetch task data and recreate the card
                    fetch(`/tasks/${taskId}`)
                        .then(response => response.json())
                        .then(taskData => {
                            // Map the status from backend to frontend format
                            let mappedStatus = newStatus;
                            
                            // Update task status in the database
                            updateTaskStatus(taskId, newStatus);
                            
                            // Create a new card with the task data from the server
                            const newCard = createTaskCard({ 
                                ...taskData, 
                                status: mappedStatus,
                                subtasks: originalCard._subtasksData || taskData.subtasks || []
                            });
                        ...
                        .catch(error => {
                            ...
                            // If there's an error, just create a simple version of the card
                            const simpleCard = document.createElement('div');
                            simpleCard.className = 'task-card';
                            simpleCard.setAttribute('data-task-id', taskId);
                            simpleCard.innerHTML = `<div class="task-title">${originalCard.querySelector('.task-title').textContent}</div>`;
                            
                            list.appendChild(simpleCard);
                            updateTaskStatus(taskId, newStatus);
                            updateTaskCounts();
                        });
                } catch (err) {
