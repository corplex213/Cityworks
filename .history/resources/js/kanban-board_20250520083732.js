...
window.loadTasksIntoKanban = function(tasks, userId) {
    // Only keep main tasks for this user
    const userTasks = tasks.filter(
        task => task.assigned_user && String(task.assigned_user.id) === String(userId) && !task.parent_task_id
    );

    // Clear all Kanban columns for this user
    ...
    // Add main tasks to appropriate columns
    userTasks.forEach(task => {
        // Ensure subtasks are up-to-date (filter out deleted subtasks)
        if (Array.isArray(task.subtasks)) {
            // Remove any subtasks that are not present in the latest tasks array
            const allSubtasks = tasks.filter(t => t.parent_task_id === task.id);
