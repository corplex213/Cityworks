...
const closeTaskDetailsDrawer = () => taskDrawerHandler.close();
    ...
    function insertMention(name) {
        ...
        if (lastAtIndex !== -1) {
            ...
            // Ensure the username is wrapped in a specific format that can be easily identified
    ...
    function handleCommentInput(event) {
        ...
        if (previewDiv) {
            if (text.includes('@')) {
                // Show preview with highlighted mentions
                previewDiv.classList.remove('hidden');
                previewDiv.innerHTML = formatMentions(text);
            } else {
