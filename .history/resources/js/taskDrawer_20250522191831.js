...
const closeTaskDetailsDrawer = () => taskDrawerHandler.close();
    ...
    function insertMention(name) {
        ...
        if (lastAtIndex !== -1) {
            ...
            updateMentionPreview(textarea.value);
    ...
    function updateMentionPreview(text) {
        const previewDiv = document.getElementById('mentionPreview');
        if (!previewDiv) return;
        
        // Show the preview and transform the text
        previewDiv.classList.remove('hidden');
        
        // Replace mentions with highlighted spans
        const highlightedText = text.replace(/@([a-zA-Z0-9 ]+?)(?=\s|$|[.,!?])/g, 
            '<span class="bg-blue-500/20 text-blue-600 dark:text-blue-400 rounded px-1">@$1</span>');
        
        previewDiv.innerHTML = highlightedText;
    }

    function handleCommentInput(event) {
        updateMentionPreview(event.target.value);
    }
...
function formatMentions(text) {
    ...
    // Match @username including spaces within the username until a space after the username or end of line
    return safeText.replace(/@([a-zA-Z0-9 ]+?)( |$)/g, '<span class="text-blue-500 dark:text-blue-500">@$1</span>$2');
}
...
window.updateMentionPreview = updateMentionPreview;
