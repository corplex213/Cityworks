...
const closeTaskDetailsDrawer = () => taskDrawerHandler.close();
                ...
                <div class="flex-grow">
                    ...
                    <div id="replyForm-${comment.id}" class="hidden mt-4">
                        <div class="flex items-start space-x-3">
                            ...
                            <div class="flex-grow relative"> <!-- Add 'relative' class -->
                                <textarea 
                                ...
                                <div id="mentionSuggestions-reply-${comment.id}" class="hidden absolute z-50 w-full bg-gray-700 border border-gray-600 rounded-lg shadow-lg mt-1 max-h-48 overflow-y-auto">
    ...
    function updateMentionSuggestionsPosition(commentId = null) {
        // Handle both main comments and replies
        const isReply = commentId !== null;
        const textareaId = isReply ? `replyText-${commentId}` : 'newComment';
        const suggestionsId = isReply ? `mentionSuggestions-reply-${commentId}` : 'mentionSuggestions';
        ...
        if (textarea && suggestionsDiv) {
            // Get textarea position
            const rect = textarea.getBoundingClientRect();
            
            // Position the suggestions div right below the textarea
            suggestionsDiv.style.position = 'absolute';
            suggestionsDiv.style.top = `${rect.bottom + window.scrollY}px`;
            suggestionsDiv.style.left = `${rect.left + window.scrollX}px`;
            suggestionsDiv.style.width = `${rect.width}px`;
            suggestionsDiv.style.zIndex = '1000'; // Ensure it's above other content
        }
    }

    // Modify the showMentionSuggestions function to include positioning
    function showMentionSuggestions(commentId = null) {
        ...
        updateMentionSuggestionsPosition(commentId);
