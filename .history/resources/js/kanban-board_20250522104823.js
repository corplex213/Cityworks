...
function initializeKanban() {
    ...
    document.querySelectorAll('.task-list').forEach(list => {
        ...
        list.addEventListener('drop', e => {
            ...
            if (originalCard) {
                try {
                    // Instead of extracting data from the DOM, fetch the task data from the server
                    fetch(`/tasks/${taskId}`)
                        ...
                        .then(taskData => {
                            ...
                            // Use the newStatus for the UI display
                        ...
                        .catch(error => {
                            ...
                            // Fallback: Just move the original card without rebuilding it
                            list.appendChild(originalCard);
                            updateTaskStatus(taskId, newStatus);
                            updateTaskCounts();
                        });
                } catch (err) {
                    console.error('Error during drag and drop:', err);
                    // Fallback: Just move the original card
                    list.appendChild(originalCard);
                    updateTaskStatus(taskId, newStatus);
                    updateTaskCounts();
                }
