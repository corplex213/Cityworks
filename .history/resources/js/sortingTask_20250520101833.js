    ...
    function sortTable(columnIndex = null, sortOrder = 'asc') {
        ...
        for (let i = 0; i < rows.length; i++) {
            ...
            if (!row.classList.contains('subtask-row')) {
                ...
                if (group.length > 1) {
                    // Exclude the main task (group[0]), sort only subtasks (group[1...])
                    let subtasks = group.slice(1);
                    // Example: sort by Task name ascending (you can adapt for other columns)
                    subtasks.sort((a, b) => {
                        // Change this logic if sorting by other columns
                        const aVal = a.querySelector('td:first-child input')?.value.trim() || '';
                        const bVal = b.querySelector('td:first-child input')?.value.trim() || '';
                        return aVal.localeCompare(bVal, undefined, { numeric: true });
                    });
                    // Rebuild the group: [main task, ...sorted subtasks]
