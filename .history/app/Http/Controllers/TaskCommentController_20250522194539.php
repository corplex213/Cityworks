            ...
            if (!empty($matches[1])) {
                foreach ($matches[1] as $mentionedName) {
                    ...
                    if ($mentionedUser && $mentionedUser->id !== Auth::id()) {
                        // Add to the collection of users to notify (without duplicates)
                        if (!$usersToNotify->contains('id', $mentionedUser->id)) {
                            $usersToNotify->push($mentionedUser);
                        }
                        
                        // Create a specific mention notification (separate from general task notifications)
                        $this->notificationService->createForUser(
                            $mentionedUser->id,
                            'mention',
                            'You were mentioned',
                            Auth::user()->name . ' mentioned you in a comment on task "' . $task->task_name . '"',
                            route('projects.show', $task->project_id) // Navigate to the project page instead
                        );
                    }
                }
            }
