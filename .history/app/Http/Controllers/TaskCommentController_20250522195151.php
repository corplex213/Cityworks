...
class TaskCommentController extends Controller
{
    ...
    {
        try {
            // Log the comment content to check what we're processing
            ...
            \Log::info('Mention matches: ', $matches[1] ?? []);
            ...
            if (!empty($matches[1])) {
                foreach ($matches[1] as $mentionedName) {
                    ...
                    if ($mentionedUser && $mentionedUser->id !== Auth::id()) {
                        // Create a SEPARATE mention notification
                        \Log::info('Creating mention notification for user: ' . $mentionedUser->name);
                        
                        $this->notificationService->createForUser(
                            $mentionedUser->id,
                            'mention',
                            'You were mentioned',  // This is the title that should appear
