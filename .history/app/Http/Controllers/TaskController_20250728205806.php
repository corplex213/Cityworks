    ...
    public function store(Request $request)
    {
        try {
            ...
            Activity::create([
                ...
                'description' => "Created task: {$task->task_name}",
                'properties' => [
        ...
        try {
            ...
            if (!empty($changes)) {
                Activity::create([
                    ...
                    'subject_id' => $task->id,
                    'subject_type' => Task::class,
                    'description' => "Updated task: {$task->task_name}",
                    'properties' => [
                    ...
                    if (!empty($subtaskData['id']) && in_array($subtaskData['id'], $existingSubtaskIds)) {
                        ...
                        if (!empty($subtaskChanges)) {
                            ...
                            Activity::create([
                                ...
                                'subject_id' => $subtask->id,
                                'subject_type' => Task::class,
                                'description' => "Updated subtask: {$subtask->task_name} of task \"{$task->task_name}\"",
                                'properties' => [
                                    'project_id' => $task->project_id,
        ...
        try {
            ...
            if ($isSubtask) {
                ...
                Activity::create([
                    ...
                    'subject_id' => $task->id,
                    'subject_type' => Task::class,
                    'description' => "Deleted subtask: {$task->task_name}",
                    'properties' => [
                        'project_id' => $task->project_id,
                        'changes' => [
                ...
                Activity::create([
                    ...
                    'subject_id' => $task->id,
                    'subject_type' => Task::class,
                    'description' => "Deleted task: {$task->task_name}",
                    'properties' => [
                        'project_id' => $task->project_id,
                        'changes' => [
                            'task_name' => ['old' => $task->task_name],
