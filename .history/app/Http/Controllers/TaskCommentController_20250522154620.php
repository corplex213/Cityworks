...
class TaskCommentController extends Controller
{
    ...
    {
        try {
            // Extract all @username patterns from the comment
            preg_match_all('/@([a-zA-Z0-9 ]+?)(?=\s|$|[.,!?])/', $comment->content, $matches);
            
            \Log::info('Mention detection: ', ['matches' => $matches[1]]);

            if (!empty($matches[1])) {
                foreach ($matches[1] as $username) {
                    // Find the user by name
                    $mentionedUser = User::where('name', trim($username))->first();
                    
                    \Log::info('Looking for user: ' . trim($username) . ' - Found: ' . ($mentionedUser ? 'YES' : 'NO'));

                    if ($mentionedUser && $mentionedUser->id !== Auth::id()) {
                        \Log::info('Creating mention notification for: ' . $mentionedUser->name);
                        // Add to users to notify if not already included
                        if (!$usersToNotify->contains('id', $mentionedUser->id)) {
                            $usersToNotify->push($mentionedUser);
                        }
                        
                        // Create a specific mention notification
                        $this->notificationService->createForUser(
                            $mentionedUser->id,
                            'mention',
            ...
            // Log the error but don't break the comment submission
