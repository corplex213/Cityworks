            ...
            case 'subtask_updated':
                ...
                if (isset($decodedChanges['due_date'])) {
                    ...
                    if (!empty($oldDate) && !empty($newDate)) {
                        $oldDate = date('M d, Y', strtotime($oldDate));
                        $newDate = date('M d, Y', strtotime($newDate));
                    }
                    $messages[] = "<strong>{$userName}</strong> changed due date of subtask \"{$taskName}\" from \"{$oldDate}\" to \"{$newDate}\"";
                }
                ...
                if (isset($decodedChanges['budget'])) {
                    $oldBudget = $decodedChanges['budget']['old'] ?? 0;
                    $newBudget = $decodedChanges['budget']['new'] ?? 0;
                    $oldFormatted = number_format($oldBudget, 2);
                    $newFormatted = number_format($newBudget, 2);
                    $messages[] = "<strong>{$userName}</strong> changed budget of subtask \"{$taskName}\" from \"₱{$oldFormatted}\" to \"₱{$newFormatted}\"";
                }
                
                if (isset($decodedChanges['source_of_funding'])) {
                    $oldSource = $decodedChanges['source_of_funding']['old'] ?? 'None';
                    $newSource = $decodedChanges['source_of_funding']['new'] ?? 'None';
                    $messages[] = "<strong>{$userName}</strong> changed funding source of subtask \"{$taskName}\" from \"{$oldSource}\" to \"{$newSource}\"";
                }

                if (isset($decodedChanges['other_funding_source'])) {
                    $oldOtherSource = $decodedChanges['other_funding_source']['old'] ?? 'None';
                    $newOtherSource = $decodedChanges['other_funding_source']['new'] ?? 'None';
                    $messages[] = "<strong>{$userName}</strong> changed other funding source of subtask \"{$taskName}\" from \"{$oldOtherSource}\" to \"{$newOtherSource}\"";
                }
