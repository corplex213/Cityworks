    ...
    public function getFormattedMessage()
    {
        ...
        switch ($this->type) {
            ...
            case 'subtask_updated':
                $parentTaskName = $decodedChanges['parent_task_name'] ?? 'unknown task';
                // Only one field per activity (enforced by controller)
                foreach ($decodedChanges as $field => $change) {
                    ...
                    switch ($field) {
                        case 'task_name':
                            return "<strong>{$userName}</strong> changed Task name from \"{$old}\" to \"{$new}\" in subtask \"{$new}\" of task \"{$parentTaskName}\"";
                        case 'start_date':
